...
/**
 * Port of CE's {@code com.hbm.render.anim.sedna.BusAnimationKeyframeSedna} (606 lines) - one
 * keyframe on one {@link BusAnimationSequenceSedna.Dimension} channel of one bus: a target
 * {@link #value}, a {@link #duration} in (animation-speed-scaled) milliseconds, an
 * {@link IType interpolation type}, and (for the Blender-ported curve families) an
 * {@link EType easing} direction plus whatever extra per-curve parameters that family needs
 * (bezier handles, elastic amplitude/period, back overshoot).
 *
 * <p>This class was flagged by {@code docs/phase5/weapon_gun_rendering_animloader.md} as
 * <b>not</b> read line-by-line in that report (time-budgeted against the structurally-identical
 * legacy {@code BusAnimationKeyframe}, which was read in full) - this port re-reads and ports
 * CE's real Sedna file directly, in full, confirming it is indeed structurally and numerically
 * identical to the legacy class (same 17 {@link IType} values, same 4 {@link EType} values, same
 * {@code BLI_easing_*} function bodies verbatim from Blender's {@code easing.c}, same
 * {@link #interpolate} dispatch and cubic-bezier root-finding math) - no behavioral differences
 * found between the two CE classes.
 *
 * <p><b>Kept as {@code double}, not {@code float}</b>, per the phase-5 doc's explicit "either is
 * fine, pick one and be consistent" - {@code double} was chosen because it is CE's own type
 * throughout the whole engine ({@link BusAnimationSequenceSedna#getTransformation} also returns
 * {@code double[]}) and minimises the risk of a subtle bezier-root-finding precision difference
 * creeping into a line-by-line numeric port; Neo Edition instead chose {@code float} for its own
 * port of the legacy (non-Sedna) engine, an equally valid but different choice not followed here.
 *
 * <p>{@link ClientConfig#GUN_ANIMATION_SPEED} (already committed, a {@code DoubleValue} - this
 * port's own {@code com.hbm.config.ClientConfig}, not CE's {@code ConfigWrapper}-based one) is
 * read exactly where CE reads its own equivalent field, in the 2-arg constructor - confirmed
...
public class BusAnimationKeyframeSedna {
    ...
    /** Bezier handle type (parsed from the JSON, not otherwise consumed by the math below - matches CE, which stores but never branches on it either). */
    public enum HType {
        FREE,
        ALIGNED,
        VECTOR,
    ...
    /** "Reset" keyframe - a neutral, zero-duration linear frame, mirroring CE's own no-arg constructor use as {@link #interpolate}'s implicit "no previous frame" default. */
     ...
     * not its end, matching how {@link BusAnimationSequenceSedna#getTransformation} walks the list.
     */
