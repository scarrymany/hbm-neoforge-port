...
/**
 * Port of CE's {@code com.hbm.render.anim.BusAnimationKeyframe} (566 lines) - one keyframe on one
 * {@link BusAnimationSequence.Dimension} channel of one bus: a target {@link #value}, a
 * {@link #duration} in (animation-speed-scaled) milliseconds, an {@link IType interpolation type},
 * and (for the Blender-ported curve families) an {@link EType easing} direction plus whatever
 * extra per-curve parameters that family needs (bezier handles, elastic amplitude/period, back
 * overshoot).
 *
 * <p>This is the older, hand-authored-in-Java sibling of the JSON-driven
 * {@link com.hbm.render.anim.sedna.BusAnimationKeyframeSedna}, ported separately and independently
 * - CE genuinely maintains both engines side by side (Sedna's own class javadoc: "this duplicate
 * abomination was done solely for sedna guns"). {@code docs/phase5/weapon_gun_rendering_animloader.md}
 * already confirmed the two CE classes are structurally and numerically identical (same 17
 * {@link IType} values, same 4 {@link EType} values, same {@code BLI_easing_*} bodies verbatim from
 * Blender's {@code easing.c}, same {@link #interpolate} dispatch and cubic-bezier root-finding
 * math) - this port re-reads and ports CE's real legacy file directly, confirming that finding.
 * Unlike the Sedna sibling, this class keeps CE's legacy {@link #BusAnimationKeyframe(double,
 * double, double, int) triplet constructor}, {@link #toArray()}, and the {@link InterpolationType}
 * enum - real CE API on <i>this</i> class (hand-authored Java animations build combined XYZ
 * keyframes directly), not carried over onto the JSON-only Sedna side because JSON authoring never
 * needs it.
 *
 * <p><b>Kept as {@code double}, not {@code float}</b>, matching CE's own type throughout the whole
 * engine and the already-committed Sedna sibling's same choice (see that class's javadoc) - this
 * minimises the risk of a subtle bezier-root-finding precision difference creeping into a
 * line-by-line numeric port.
 *
 * <p>{@link ClientConfig#GUN_ANIMATION_SPEED} (already committed, a {@code DoubleValue} - this
 * port's own {@code com.hbm.config.ClientConfig}, not CE's {@code ConfigWrapper}-based one) is read
 * exactly where CE reads its own equivalent field, in the duration-scaling constructors - confirmed
 * present and correctly typed before writing this class.
 */
public class BusAnimationKeyframe {
    ...
    /** Legacy (pre-{@link IType}) constant-vs-lerp switch, still exposed on the triplet constructor for hand-authored callers. */
    ...
    /** Bezier handle type (parsed/stored, not otherwise consumed by the math below - matches CE, which stores but never branches on it either). */
    public enum HType {
        FREE,
        ALIGNED,
        VECTOR,
    ...
    /** "Reset" keyframe - a neutral, zero-duration linear frame, used both as {@link #interpolate}'s implicit "no previous frame" default and by {@link BusAnimationSequence#hold}. */
    ...
    /** Combined XYZ keyframe - stored in {@link #legacyX}/{@link #legacyY}/{@link #legacyZ} and expanded into three per-axis keyframes by {@link BusAnimationSequence#addKeyframe(BusAnimationKeyframe)}. */
    ...
    /**
     * Samples this keyframe's segment (from {@code previous} up to this frame) at {@code currentTime},
     * dispatching to the matching interpolation family. Verbatim port of CE's own dispatch, including
     * the "constant value optimisation" early-out and the bezier/BLI-easing branches all keying off
     * {@code previous.interpolationType}/{@code previous.easingType} (not this frame's own type) -
     * a real, non-obvious CE quirk: a segment's shape is determined by the keyframe at its <i>start</i>,
     * not its end, matching how {@link BusAnimationSequence#getTransformation} walks the list.
     */
    ...
    // Blender bezier solvers, but rewritten (pain) - comment preserved verbatim from CE.
    ...
    // ---- EASING FUNCTIONS, taken directly from Blender `easing.c` (verbatim port, CE's own comment) ----
