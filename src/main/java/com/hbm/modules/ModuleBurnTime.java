...
/**
 * Exact CE {@code com.hbm.modules.ModuleBurnTime} — fuel-class time/heat bonuses.
 * JSON keys Exact CE {@code :42-78} including {@code D:heatBalefie} typo.
 * {@code TileEntityFurnace.getItemBurnTime} → {@code ItemStack#getBurnTime(SMELTING)}.
 * OreDict names → tags + path fragments so CE {@code contains("Coal")}/{@code startsWith("log")} still hit.
 */
public class ModuleBurnTime {
    ...
    public double getMod(ItemStack stack, double[] mod, double def) {
        ...
        // CE ModuleBurnTime.java:103-111 — ModItems identity
        if (isHbm(stack, "solid_fuel")) return mod[modSolid];
        if (isHbm(stack, "solid_fuel_presto")) return mod[modSolid];
        if (isHbm(stack, "solid_fuel_presto_triplet")) return mod[modSolid];

        if (isHbm(stack, "solid_fuel_bf")) return mod[modBalefire];
        if (isHbm(stack, "solid_fuel_presto_bf")) return mod[modBalefire];
        if (isHbm(stack, "solid_fuel_presto_triplet_bf")) return mod[modBalefire];

        if (isHbm(stack, "rocket_fuel")) return mod[modRocket];

        for (String name : oreNames(stack)) {
    ...
    private static boolean isHbm(ItemStack stack, String path) {
        Item item = BuiltInRegistries.ITEM.get(ResourceLocation.fromNamespaceAndPath(MainRegistry.MODID, path));
        return stack.getItem() == item;
    }

    /** Tag ids plus path / last-segment so CE {@code contains}/{@code startsWith} still match 1.21 tags. */
