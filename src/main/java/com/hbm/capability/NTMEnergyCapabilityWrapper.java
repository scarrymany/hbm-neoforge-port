...
/**
 * Wraps {@link IEnergyHandlerMK2} (NTM's own HE tile-entity energy API) as {@link IEnergyStorage}
 * for Forge/NeoForge Energy compatibility. Ported unchanged from CE's
 * {@code NTMEnergyCapabilityWrapper}, aside from {@code TileEntity} -> {@link BlockEntity} and the
 * capability interface's package.
 */
public class NTMEnergyCapabilityWrapper implements IEnergyStorage {
     ...
     */
    public NTMEnergyCapabilityWrapper(@NotNull BlockEntity handler, @Nullable BlockPos pos) {
        if (handler instanceof IEnergyHandlerMK2 energyHandlerMK2) this.handler = energyHandlerMK2;
        else throw new IllegalArgumentException("BlockEntity " + handler.getClass().getName() + " must implement EnergyHandlerMK2");
    ...
    public NTMEnergyCapabilityWrapper(@NotNull BlockEntity handler) {
        this(handler, null);
    }
