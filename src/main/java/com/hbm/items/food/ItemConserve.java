 ...
 * CE). {@link FoodType#FIST} deals 2 magic damage to the eater ({@code DamageSource.MAGIC}), Exact CE
 * {@code ItemConserve.java:60-61}. {@link FoodType#RECURSION} self-duplicates 90% of the time.
 */
    ...
    public ItemStack finishUsingItem(ItemStack stack, Level level, LivingEntity entity) {
        ...
        if (!level.isClientSide()) {
            switch (type) {
                ...
                case FIST -> entity.hurt(entity.damageSources().magic(), 2F);
                case RECURSION -> {
