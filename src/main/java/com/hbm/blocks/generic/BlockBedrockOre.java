 ...
 * constructor instead of re-deriving it from a field-identity check.
 */
public class BlockBedrockOre extends Block implements IDrillInteraction {

    private final Item drillDrop;

    public BlockBedrockOre(Properties properties, Item drillDrop) {
        super(properties);
    ...
    public ItemStack extractResource(Level world, int x, int y, int z, BlockState state, IMiningDrill drill) {
        ...
        return world.getRandom().nextInt(50) == 0 ? new ItemStack(drillDrop) : ItemStack.EMPTY;
    }
