 ...
 * Crucible - see {@code ProcessingClientRegistry}'s own javadoc for why this is a separate
 * {@code @EventBusSubscriber} rather than an edit to {@code ClientModRegistry#registerScreens} (same
 * multi-area-in-one-wave race that class avoids).
 */
...
public final class CrucibleClientRegistry {

    private CrucibleClientRegistry() {
    }
