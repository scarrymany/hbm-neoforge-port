...
public class ItemRenderHeavyRevolver extends ItemRenderGunBase {
	...
	protected void renderFirstPerson(ItemStack stack, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, int packedOverlay) {
		...
		poseStack.mulPose(Axis.ZP.rotationDegrees((float) ((float) spin[0])));
		...
		poseStack.mulPose(Axis.ZP.rotationDegrees((float) ((float) (recoil[2] * -10))));
		/* TODO(CE:ItemRenderHeavyRevolver:smokeNodes) smokeNodes not ported */
		poseStack.popPose();
		...
		poseStack.mulPose(Axis.ZP.rotationDegrees((float) (-30 + 30 * (float) GunAnimationClientState.getRelevantTransformation("HAMMER")[2])));
		poseStack.translate(-4, -1.25, 0);
		renderPart(GunModels.obj("lilmac"), poseStack, bufferSource, currentTex, packedLight, packedOverlay, "Hammer");
		poseStack.popPose();

		if (isScoped) {
			/* bind */ currentTex = GunModels.tex("lilmac_scope_tex");
			renderPart(GunModels.obj("lilmac"), poseStack, bufferSource, currentTex, packedLight, packedOverlay, "Scope");
		}
	...
	public boolean isScoped(ItemStack stack) {
		return stack.getItem() == GunPistolItems.GUN_HEAVY_REVOLVER_LILMAC.get() || XWeaponModManager.hasUpgrade(stack, 0, ResourceLocation.fromNamespaceAndPath(MainRegistry.MODID, "scope"));
	}
}
